function [mix] = gmm_beta(y,m,verbose, BE)
...
%                  In the field prior:
%
% lambda_0         Prior mixers, p(pi) = D(lambda_0) (this is a0)
...
addpath(genpath('~/Dropbox/Herman/tools/spm12'))
if nargin < 3
    verbose=1;
end
[N,d]=size(y);
...
for loops=1:max_loops,
    ...
    fm=(1/BE)*(f1+sum(f2)+sum(f3))+sum(f4)+sum(f5);
